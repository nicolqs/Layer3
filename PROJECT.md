...
- **Result**: Factory pattern makes adding chains trivial
...
2. **WebSockets** - Replace SSE for better real-time
3. **Rate Limiting** - Per-user limits
4. **Monitoring** - Sentry for errors, Datadog for APM / Logs
5. **CDN** - Cache static assets, optimize images

**For Scale (10K+ users):**

- **Redis cluster** - Shared cache, 90% fewer API calls, biggest cost savings
- **PostgreSQL** - Indexed transactions, pre-computed stats, read replicas
- **Background workers** - Separate data refresh from web servers, prioritize active users
- **Monitoring** - APM (Datadog) + error tracking (Sentry) to identify bottlenecks
